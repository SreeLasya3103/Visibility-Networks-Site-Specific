"""
4-stream dataset: outputs [original, pseudo-colored, FFT, diff-from-reference]
as a [4, 3, H, W] tensor. Keeps FFT stream intact, ADDS diff as 4th.
"""
...
class DsetClass(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Keep cropped+resized RGB copy for diff
        current_rgb = sample_data.clone()

        # Standard 3-stream transformer -> [3, 3, H, W]  (orig, PC, FFT)
        if self.transformer:
            sample_data = self.transformer(sample_data)

        # Compute diff as 4th stream
        site_id = self._get_site_id(rel_sample_path)
        ref_img = self._load_reference(site_id)

        if ref_img is not None:
            diff = torch.abs(current_rgb - ref_img).clamp(0.0, 1.0)  # [3, H, W]
        else:
            diff = torch.zeros_like(current_rgb)

        # Append as 4th stream -> [4, 3, H, W]
        sample_data = torch.cat([sample_data, diff.unsqueeze(0)], dim=0)
